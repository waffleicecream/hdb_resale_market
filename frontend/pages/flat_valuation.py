...
def run_valuation(n_submit, n_demo, postal, flat_type, storey_bin, lease_bin, listed):  # noqa: ARG001
    ...
    # Always use the block's actual lease from our data — ignore user-supplied value
    meta = _POSTAL_META.get(str(postal).strip().zfill(6))
    actual_lease = meta["remaining_lease_bin"] if meta else None
    lease_bin = actual_lease or lease_bin  # fall back to form value only if we have no data
